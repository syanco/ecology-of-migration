...
```{r}
load("../out/extract_2021-01-21.Rdata")
source("./functions.R")
```
...
temp_df %>%
  ...
  add_predicted_draws(fit_temp, n=1000) %>%
  summarise(
    p_residual = mean(.prediction < temp),
...
# macf(residuals(fit_temp, summary = FALSE), lag.max = 10)
```

```{r}
mcmc_plot(fit_temp, type = "acf_bar")
```
...
Margianls
```{r}
mmp_brm(fit_temp, x = "season", prob = .95)
```
...
fit_ndvi <- brm(bf(ndvi ~ 0 + season + (1|bird) + ar(time = date2, gr = bird, cov = T), 
             phi ~ 0 + season, family = Beta()), data = ndvi_df, 
             chains = 2, iter = 1000, cores = getOption("mc.cores", 3),
...
ndvi_df %>%
  mutate(date = as.numeric(date)) %>% 
  add_predicted_draws(fit_ndvi, n=1000) %>%
  summarise(
    p_residual = mean(.prediction < ndvi),
    z_residual = qnorm(p_residual)
  ) %>%
  ggplot(aes(sample = z_residual)) +
  geom_qq() +
...
fit_ndvi_2 <- brm(bf(ndvi ~ 0 + season + (1|bird) + ar(time = date2, gr = bird, cov = F), 
             sigma ~ 0 + season, family = gaussian()), data = ndvi_df, 
             chains = 4, iter = 1000, cores = getOption("mc.cores", 3)) 

fit_ndvi_2
```
```{r}
resid <- resid(fit_ndvi_2, type = "pearson")[, "Estimate"]
n <- length(resid)
resid[-1] <- resid[-1] - (resid[-n] * mean(as.matrix(fit_ndvi_2)[,
    "ar[1]"]))
fit <- fitted(fit_ndvi_2)[, "Estimate"]
...
ndvi_df %>%
  mutate(date = as.numeric(date)) %>% 
  add_predicted_draws(fit_ndvi_2, n=1000) %>%
  summarise(
    p_residual = mean(.prediction < ndvi),
    z_residual = qnorm(p_residual)
  ) %>%
...
Prior predictive check
```{r}
pp_check(fit_ndvi_2, nsamples = 100)
```
