...
(rawtemps <- ggplot() +
  ...
  scale_color_brewer(palette = "Set1") +
  annotate(geom="text", x=1, y=20, label="A", size = 7) +
...
(meantemps <- ggplot(temp_mod_df[temp_mod_df$param == "mu",]) +
  ...
  width =.5, size = .5) +
  annotate(geom="text", x=4, y=14.7, label="B", size = 7) +
...
(SDtemps <- ggplot(temp_mod_df[temp_mod_df$param == "sigma",]
  ...
  scale_colour_brewer(palette = "Set1") +
  scale_x_discrete(labels = c("Spring", "Summer", "Fall", "Winter")) +
  annotate(geom="text", x=4.2, y=7, label="C", size = 7) +
...
```
```{r}
ggsave("../figures/rawNT.png", rawhypoNT, width = 3.5, height = 3)
```
...
```{r}
ggsave("../figures/muNT.png", meanhypoNT, width = 3.5, height = 3)
```
...
```
```{r}
ggsave("../figures/rawNDVI.png", rawhypoNDVI, width = 3.5, height = 3)
```
...
(meanhypoNDVI <- ggplot() +
    ...
    #real points and error bars
    geom_errorbar(data = merge_ndvi[merge_ndvi$type == "real",],
                  aes(x = season, ymax = mu.CI.h, ymin = mu.CI.l, group = bird), 
                  width = .3, size = 1, color = "black", alpha = 0.4,
                  position=position_dodge(width=0.9)) +
    geom_point(data = merge_ndvi[merge_ndvi$type == "real",],
               aes(x = season, y = mu, group = bird),
               shape = 17, size = 1.5, color = "black", alpha = 0.4,
               position=position_dodge(width=0.9)) +
    
    #simulated points and bars
    geom_errorbar(data = merge_ndvi[merge_ndvi$type == "hypo",],
                  aes(x = season, ymax = mu.CI.h, ymin = mu.CI.l, 
                      color = season, group = bird),
                  width = .3, size = 1, position=position_dodge(width=0.9)) +
    geom_point(data = merge_ndvi[merge_ndvi$type == "hypo",],
               aes(x = season, y = mu, color = season, group = bird),
               size = 1.5, position=position_dodge(width=0.9)) +
...
```
```{r}
ggsave("../figures/muNDVI.png", meanhypoNDVI, width = 3.5, height = 3)
```

Get mean pairwise difference:
```{r}
mean_pair_ndvi <- merge_ndvi %>% 
  pivot_wider(id_cols = bird, names_from = c(season, type), values_from = mu)
...
```
```{r}
ggsave("../figures/legend.png", legend, width = 7, height = 0.5)
```
...
```{r}
ggsave("../figures/Hypo_Wint/rawNT.png", rawhypoNTwint, width = 3.5, height = 3)
```
...
(meanhypoNTwint <- ggplot() +
   ...
   #real points and error bars
   geom_errorbar(data = merge_temps_wint[merge_temps_wint$type == "real",],
                 aes(x = season, ymax = mu.CI.h, ymin = mu.CI.l, group = bird), 
                 width = .5, size = .5, color = "grey44",
                 position=position_dodge(width=0.9)) +
   geom_point(data = merge_temps_wint[merge_temps_wint$type == "real",],
              aes(x = season, y = mu, group = bird),
              shape = 17, size = 1.5, color = "grey44",
              position=position_dodge(width=0.9)) +
   
   #simulated points and bars
   geom_errorbar(data = merge_temps_wint[merge_temps_wint$type == "hypo",],
                 aes(x = season, ymax = mu.CI.h, ymin = mu.CI.l, 
                     color = season, group = bird),
                 width = .5, size = .5, position=position_dodge(width=0.9)) +
   geom_point(data = merge_temps_wint[merge_temps_wint$type == "hypo",],
              aes(x = season, y = mu, color = season, group = bird),
              size = 1.5, position=position_dodge(width=0.9)) +
...
```
```{r}
ggsave("../figures/Hypo_Wint/meanNT.png", meanhypoNTwint, width = 3.5, height = 3)
```
Extract legend.

```{r}
hypNTlegwint <- ggplot(merge_temps_wint) +
  geom_point(data = merge_temps_wint,
             aes(x = season, color = season, y = mu, 
                 shape = type, alpha = type)) +
  geom_errorbar(
    aes(x = season, color = season, ymax = mu.CI.h, ymin = mu.CI.l,
        alpha = type),
    width = .3) +
  scale_color_brewer(palette = "Set1", 
                     name = "Season") +
  scale_x_discrete(labels = c("Winter", "Spring", "Fall")) +
  scale_alpha_manual(values = c(1, 0.3), 
                     labels = c("Simulated", "Experienced"),
                     name = "Data Type") +
  scale_shape_discrete(labels = c("Simulated", "Experienced"),
...
legend <- g_legend(hypNTlegwint)
grid.arrange(legend)
```

### Arrange into single
```{r fig.height=10, fig.width=7}
gl5 <- list(rawhypoNTwint, meanhypoNTwint, legend)

(hypontwintfig <- arrangeGrob(
  grobs = gl5,
  heights = unit(c(3, 5), c("in", "in")),
  #  widths = c(2, 1, 1),
  layout_matrix = rbind(c(1, 3),
                        c(2, 2))
))

ggsave("hypo_nt_wint_fig.pdf", hypontwintfig)
ggsave("hypo_nt_wint_fig.png", hypontwintfig)
```
...
```{r}
ggsave("../figures/Hypo_Wint/rawNNDVI.png", rawhypoNDVIwint, width = 3.5, height = 3)
```
...
```
```{r}
ggsave("../figures/Hypo_Wint/meanNDVI.png", meanhypoNDVIwint, width = 3.5, height = 3)
```
Extract legend.

```{r}
hypNDVIlegwint <- ggplot(merge_ndvi_wint) +
  geom_point(aes(x = season, color = season, y = mu, 
                 shape = type, alpha = type)) +
  geom_errorbar(aes(x = season, color = season, ymax = mu.CI.h, ymin = mu.CI.l,
                    alpha = type),
                width = .3) +
  scale_color_brewer(palette = "Set1", 
                     name = "Season") +
...
legend <- g_legend(hypNDVIlegwint)
grid.arrange(legend)
```

### Arrange into single
```{r fig.height=10, fig.width=7}
gl6 <- list(rawhypoNDVIwint, meanhypoNDVIwint, legend)

(hypondviwintfig <- arrangeGrob(
  grobs = gl6,
  heights = unit(c(3, 5), c("in", "in")),
  #  widths = c(2, 1, 1),
  layout_matrix = rbind(c(1, 3),
...
ggsave("hypo_ndvi_wint_fig.pdf", hypondviwintfig)
ggsave("hypo_ndvi_wint_fig.png", hypondviwintfig)
```
