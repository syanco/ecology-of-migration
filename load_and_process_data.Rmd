...
Suppress rgdal proj4 warnings (new since switch to proj6 and WKT fromat - no behavior changed)
```{r}
...
```{r}
timing <- read.csv("../data/dep-arr_dates_07222021.csv", stringsAsFactors = F,
                   na.strings = "NA")
```
...
Estimate the breeding season departure/arrival dates as the day between the last known on-territory fix and the first fix on migration:
```{r}
...
mig_tracks <- readPinpoint(
  "../data/migration_gps/PinPoint 3083 2021-07-07 10-59-39.txt",
  birdID = "03291", start = "19/7/11 00:01:00", stop = "21/7/10 00:00:01",
  band = "1603-03291", breedyear = 2019, database = mig_tracks)
...
breed_3495 <- makeBreedVec("03495_2019", breed_loc, timing, mig_tracks, 
                           origin1 = "2019-01-01", origin2 = "2020-01-01",
                           thin = 8)
breed_3291 <- makeBreedVec("03291_2019", breed_loc, timing, mig_tracks, 
                           origin1 = "2019-01-01", origin2 = "2020-01-01",
                           thin = 8)
...
Now we can add these new breeding locations to the total data set (`mig_tracks_sf`) and then convert it to the same projection as the MODIS data (faster to convert the points than the huge raster stacks). We also clean out some intermediary objects again just to manage the global environment size.

Note, I've suppressed warnings here because it spits out a bunch about converting factors to characters.
```{r warning=FALSE}
...
rm(a41_breed, a44_breed, a43_breed, cng1_breed, bg5_breed, 
   ...
   breed_3480, breed_3482, breed_3483, breed_3291,
   mig_tracks_new)
```
...
wint_3495 <- makeWintVec("03495_2019", timing, mig_tracks_sinu,
                         dist_thresh = 1000)
wint_3291 <- makeWintVec("03291_2019", timing, mig_tracks_sinu,
                         dist_thresh = 1000)
...
Now we add these points into the total data set as well - note that because the winter imputation uses the collected data, and we did this after we had converted those data to the sinu projection, we need to add sinu to sinu here.  Again, we'll clean some objects out to manage memory.
```{r warning=FALSE}
...
rm(a41_wint, a44_wint, a43_wint, cng1_wint, b5_wint,
   ...
   wint_3480, wint_3482, wint_3483, wint_3291)
```
...
hyp_3495 <- makeHypoBreed("03495_2019", timing, mig_tracks_sinu, 
                          ...
                          hyporigin = as.Date("01/01/2020", 
                                              format = "%m/%d/%Y"), thin = 8)
hyp_3291 <- makeHypoBreed("03291_2019", timing, mig_tracks_sinu, 
                          trackorigin = as.Date("01/01/2019", 
                                                format = "%m/%d/%Y"), 
                          hyporigin = as.Date("01/01/2020", 
                                              format = "%m/%d/%Y"), thin = 8)
...
hypo_wint_3495 <- makeHypoWint("03495_2019", breed_loc, timing, 
                               origin = "2019-01-01", thin = 8)
hypo_wint_3291 <- makeHypoWint("03291_2019", breed_loc, timing, 
                               origin = "2019-01-01", thin = 8)
...
```{r message=FALSE, warning=FALSE}
#messages and warnings off just to suppress a bunch of output about default CRS

#Load the NDVI raster stack and rename it.
load("../data/ndvi_out/VI_16Days_500m_v6/Time_Series/RData/Terra/NDVI/MOD13A1_NDVI_1_2016_209_2020_RData.RData")
ndvi_ts <- raster_ts 

#Load the NDVI QA layer and rename it
load("../data/ndvi_out/VI_16Days_500m_v6/Time_Series/RData/Terra/QA_qual/MOD13A1_QA_qual_1_2016_209_2020_RData.RData")
ndvi_QA <- raster_ts

#updated version of `MODIStsp` package is resulting in errors in the RData file so we make the raster stacks manually
fnames <- list.files("../data/new_nt_out/Surf_Temp_Daily_1Km_v6/LST_Night_1km/", full.names = T)
fnamesQA <- list.files("../data/new_nt_out/Surf_Temp_Daily_1Km_v6/QAnight_mand/", full.names = T)
...
#remove any rasters with extent errors
fnames <- fnames[-which(check==F)[1]]
nt_ts <- raster::stack(fnames)

#repeat for night temp QA data
x2 <- list()
ext2 <- list() 
...
```{r}
load("../data/HC_ndvi/VI_16Days_500m_v6/Time_Series/RData/Terra/NDVI/MOD13A1_NDVI_49_2000_1_2020_RData.RData")
HC_ndvi <- raster_ts
rm(raster_ts)

load("../data/MEF_ndvi/VI_16Days_500m_v6/Time_Series/RData/Terra/NDVI/MOD13A1_NDVI_49_2000_1_2020_RData.RData")
MEF_ndvi <- raster_ts
rm(raster_ts)
```
...
index <- seq(as.Date("2016-01-01"), as.Date("2020-08-05"), by = 16)
```
...
for(i in 1:nrow(temp_df)) {
  start_breed <- timing$first_on_terr_doy[timing$id == temp_df$bird[i]]
  end_breed <- timing$last_on_terr_doy[timing$id == temp_df$bird[i]]
  start_wint <- timing$wint_arr_doy[timing$id == temp_df$bird[i]]
  end_wint <- timing$wint_dep_doy[timing$id == temp_df$bird[i]]
...
for(i in 1:nrow(ndvi_df)) {
  start_breed <- timing$first_on_terr_doy[timing$id == ndvi_df$bird[i]]
  end_breed <- timing$last_on_terr_doy[timing$id == ndvi_df$bird[i]]
  start_wint <- timing$wint_arr_doy[timing$id == ndvi_df$bird[i]]
  end_wint <- timing$wint_dep_doy[timing$id == ndvi_df$bird[i]]
  if(is.na(start_breed)) {
    start_breed <- median(na.omit(timing$first_on_terr_doy))}
  if(is.na(end_breed)) {
    end_breed <- median(na.omit(timing$last_on_terr_doy))}
...
ndvisummary <- ndvi_df[complete.cases(ndvi_df),] %>% 
  group_by(., season, bird) %>%
  summarise("mu" = mean(ndvi),
            "se" = se(ndvi),
            "sd" = sd(ndvi),
            "mu.CI.l" = bootMuCI(ndvi)[1],
            "mu.CI.h" = bootMuCI(ndvi)[2],
            "var" = var(ndvi),
            "var.ci.l" = varCI(ndvi)[1],
            "var.ci.h" = varCI(ndvi)[2]) %>% 
  ungroup() %>% 
  mutate(season = fct_relevel(season, c("summer", "fall", "winter", "spring")))
...
for(i in 1:nrow(hypo_temp_df)) {
  start_breed <- timing$first_on_terr_doy[timing$id == hypo_temp_df$bird[i]]
  end_breed <- timing$last_on_terr_doy[timing$id == hypo_temp_df$bird[i]]
  start_wint <- timing$wint_arr_doy[timing$id == hypo_temp_df$bird[i]]
  end_wint <- timing$wint_dep_doy[timing$id == hypo_temp_df$bird[i]]
  if(is.na(start_breed)) {
    start_breed <- median(na.omit(timing$first_on_terr_doy))}
  if(is.na(end_breed)) {
    end_breed <- median(na.omit(timing$last_on_terr_doy))}
...
merge_temps <- merge_temps %>% 
  ...
  filter(!is.na(sd)) #filters bird-seasons with too few obs to calc variance
# & bird != "A43_2016" & bird != "CNG1_2018")
```

Remove "unpaired" birds (no sim matching real in a season)
...
paired_temps <- merge_temps %>% 
  inner_join(paired, by = c("bird", "season"))
```
...
Gather into `data.frame`.
```{r}
ndvi_hypo_df <- data.frame("ndvi" = ndvi_hypo, 
                           "doy" = yday(as.Date(mig_tracks_hypo$date, 
                                                format = "%y/%m/%d")),
                           "date" = as.Date(mig_tracks_hypo$date, 
                                            format = "%y/%m/%d"),
...
for(i in 1:nrow(ndvi_hypo_df)) {
  start_breed <- timing$first_on_terr_doy[timing$id == ndvi_hypo_df$bird[i]]
  end_breed <- timing$last_on_terr_doy[timing$id == ndvi_hypo_df$bird[i]]
  start_wint <- timing$wint_arr_doy[timing$id == ndvi_hypo_df$bird[i]]
  end_wint <- timing$wint_dep_doy[timing$id == ndvi_hypo_df$bird[i]]
  if(is.na(start_breed)) {
    start_breed <- median(na.omit(timing$first_on_terr_doy))}
  if(is.na(end_breed)) {
    end_breed <- median(na.omit(timing$last_on_terr_doy))}
...
paired_ndvi <- merge_ndvi %>% 
  inner_join(paired_2, by = c("bird", "season"))
```
...
for(i in 1:nrow(hypo_temp_wint_df)) {
  start_breed <- timing$first_on_terr_doy[
    timing$id == hypo_temp_wint_df$bird[i]]
  end_breed <- timing$last_on_terr_doy[
    timing$id == hypo_temp_wint_df$bird[i]]
  start_wint <- timing$wint_arr_doy[timing$id == hypo_temp_wint_df$bird[i]]
  end_wint <- timing$wint_dep_doy[timing$id == hypo_temp_wint_df$bird[i]]
  if(is.na(start_breed)) {
    start_breed <- median(na.omit(timing$first_on_terr_doy))}
  if(is.na(end_breed)) {
    end_breed <- median(na.omit(timing$last_on_terr_doy))}
...
Remove "unpaired" birds (no sim matching real in a season).
```{r}
paired_3 <- merge_temps_wint %>% 
  group_by(bird, season) %>% 
  summarise(count = n()) %>% 
  filter(count == 2)

paired_temps_wint <- merge_temps_wint %>% 
  inner_join(paired_3, by = c("bird", "season"))
```
...
for(i in 1:nrow(ndvi_hypo_wint_df)) {
  start_breed <- timing$first_on_terr_doy[
    timing$id == ndvi_hypo_wint_df$bird[i]]
  end_breed <- timing$last_on_terr_doy[
    timing$id == ndvi_hypo_wint_df$bird[i]]
  start_wint <- timing$wint_arr_doy[timing$id == ndvi_hypo_wint_df$bird[i]]
  end_wint <- timing$wint_dep_doy[timing$id == ndvi_hypo_wint_df$bird[i]]
  if(is.na(start_breed)) {
    start_breed <- median(na.omit(timing$first_on_terr_doy))}
  if(is.na(end_breed)) {
    end_breed <- median(na.omit(timing$last_on_terr_doy))}
...
```
```{r}
paired_4 <- merge_ndvi_wint %>% 
  group_by(bird, season) %>% 
  summarise(count = n()) %>% 
  filter(count == 2)

paired_ndvi_wint <- merge_ndvi_wint %>% 
  inner_join(paired_4, by = c("bird", "season"))
```
...
```{r}
co_temp <- read.csv("../data/co_temp.csv")
idut_temp <- read.csv("../data/ut_id_temp.csv")

comb_temp <- rbind(co_temp, idut_temp)
```

Convert temp readings to Celsius
```{r}
#convert to C
comb_temp$tmin_c <- (comb_temp$TMIN-32)*(5/9)

#convert dates
comb_temp$DATE <- as.Date(comb_temp$DATE)

#create doy only column
comb_temp$doy <- yday(comb_temp$DATE)

#create year factor
comb_temp$y <- as.factor(year(comb_temp$DATE))
```

Break into separate study areas
```{r}
comb_temp_split <- split(comb_temp, comb_temp$NAME)
```
...
```{r}
HC_ndvi_ts <- MODIStsp_extract(in_rts = HC_ndvi, 
                               sf_object = hc_cent, 
                               out_format = "dframe")
...
```{r}
MEF_ndvi_ts <- MODIStsp_extract(in_rts = MEF_ndvi, sf_object = mef_cent, 
                                out_format = "dframe")
...
Load in eBird data
```{r}
ebd_flow <- read_ebd("../data/ebd_flaowl_relAug-2019/ebd_flaowl_relAug-2019.txt")
```
...
save(ndvi_df, temp_df, 
     ...
     comb_temp_split,
     ebd_sf,
     file = paste0("../out/extract_", Sys.Date(), ".Rdata"))
```